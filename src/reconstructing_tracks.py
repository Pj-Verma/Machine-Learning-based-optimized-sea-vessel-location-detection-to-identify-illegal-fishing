...
from src.pipeline_noLatLon import keep_columns, X_y_split
from src.utilities import get_all_data, get_group
...
def plot_track_predictions(df, mod_num, mmsi, pickeled_model, up, down, left, right):
    ...
    fig_name = '../results/best_reconstructions/' + str(mmsi) + '_transparency_' + pickeled_model[
                                                                                   len('../results/final_models/'):-len(
                                                                                       '.pkl')] + '.png'
    ...
    # m.bluemarble()
    m.drawlsmask(land_color=(2, 100 / 255, 0, 1), ocean_color='w')
    ...
    plt.plot(x, y, 'b-', alpha=0.6, linewidth=1)
    ...
    m = Basemap(projection='cyl', llcrnrlat=min_lat, urcrnrlat=max_lat, llcrnrlon=min_lon, urcrnrlon=max_lon,
                resolution='h', area_thresh=1000.)
    # m.bluemarble()
    m.drawlsmask(land_color=(2, 200 / 255, 0, 1), ocean_color='w')  # color in rgb should be gold
...
if __name__ == '__main__':
    ...
    for gear in gears:
        if gear == 'longliners':
            ...
            mod_num = 'model_1'
            pickeled_model = '../results/model_1_RF_trawlers.pkl'
            ...
            mod_num = 'model_1'
            pickeled_model = '../results/model_1_RF_purse_seines.pkl'
